...
- 히어로·도장·달력·첫인사는 카탈로그를 자동으로 읽음
- 가족사진은 합성 원본과 공개 마스크를 별도로 추가해야 함
...
5. 가족사진 합성 원본·레이어 순서·공개 영역 추가
6. `npm test`, `npm run lint`, `npm run build` 실행
7. 76px 도장, 히어로, 첫인사, 달력, 가족사진 중간 단계, 상세 팝업 직접 확인
