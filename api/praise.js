...
export function parseOpenAIResult(value) {
...
export default async function handler(request, response) {
  ...
  try {
    const aiResponse = await fetch('https://api.openai.com/v1/responses', {
      ...
      body: JSON.stringify({
        ...
        instructions: [
          ...
          '문장을 그대로 길게 반복하지 말고 20~60자 정도 한 문장으로 답한다',
    ...
    const result = parseOpenAIResult(aiResult)
    if (!result) {
